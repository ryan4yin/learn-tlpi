# Linux 桌面系统（九）：挂起、恢复与关机

前几篇依次讲了开机、登录和应用运行。离开桌面时，系统还要决定怎样保存或结束这些工作：挂起让进程暂停，关机让它们退出；休眠则把可恢复的内存状态写入持久存储。三种操作会用到部分相同的组件，最终保留的状态却不同。

桌面从 S3 恢复后，不只内存里的程序要继续运行，设备和网络连接也要重新进入可用状态。[网络篇](08-network.md)有一个网卡短暂断链并触发 networkd 重配置的案例，这里只看它与电源状态转换的关系。
...
`deep`。内核的 [System Sleep States](https://docs.kernel.org/admin-guide/pm/sleep-states.html)
...
Linux 的 swsusp 可以使用 swap 保存镜像，但配置好日常交换空间，还没有回答「下次启动怎样找到它」。恢复过程需要在挂载文件系统之前找到并读取镜像；休眠后若由另一个系统修改了同一文件系统，再恢复旧内存状态，可能破坏数据。这个约束把休眠与[启动篇](02-boot.md)里的 initramfs、存储驱动和根文件系统挂载顺序连了起来。见
...
在那个案例中，S3 恢复时 carrier 发生变化，networkd 随后重配置接口，VPN/TUN 的策略规则和出口选择也受到影响。配置分别调整了外部规则的管理边界和短暂断链的处理时机。具体配置、完整链接与取舍见[网络篇的恢复案例](08-network.md#一次恢复事件怎样影响到应用)。
...
[systemd.unit](https://man.archlinux.org/man/systemd.unit.5.en)，依赖与顺序的区别可回看[系统基础篇](03-system-foundations.md)。
...
由此判断关机卡住的位置，比套一份耗时表更有用：是某个服务仍在停止，还是已经进入最后的文件系统与存储清理？下一次启动会[重新建立系统状态](02-boot.md)，判断方法与挂起后继续运行原进程并不相同。
...
是申请者提供的说明，不是 systemd 对故障原因的诊断。若合盖没有进入睡眠，先看 `WHAT` 是否包含
`sleep`、模式是什么，再确认 logind 的合盖策略和桌面是否接管了按键。
...
结合这些信息回看[系列全景](01-architecture.md)，就能按时间梳理一台桌面的运行过程：启动时建立系统状态，应用随后使用设备与服务；挂起会暂停执行，恢复后要重新确认设备和网络状态，关机则结束进程并清理资源。
