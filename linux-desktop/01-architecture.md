...
| 文章                                                   | 沿着哪一段看                                             |
| ------------------------------------------------------ | -------------------------------------------------------- |
| [（一）系统全景与阅读路径](01-architecture.md)（本文） | 把启动、会话、应用和退出连起来                           |
| [（二）从固件到根文件系统](02-boot.md)                 | 固件怎样交给内核，系统怎样找到根文件系统                 |
| [（三）系统服务、设备与通信](03-system-foundations.md) | systemd 单元、journal、udev 和 D-Bus 怎样支撑后面的桌面  |
| [（四）登录、身份与用户会话](04-login-session.md)      | 认证之后怎样建立会话，密钥环、用户实例和设备访问如何衔接 |
| [（五）显示、输入与图形渲染](05-graphics.md)           | 输入事件怎样到达应用，应用画面怎样到达屏幕               |
| [（六）桌面应用、portal 与沙盒](06-app-integration.md) | 应用如何启动，文件访问和屏幕共享如何跨越桌面接口         |
| [（七）音频、字体与输入法](07-media-input.md)          | 声音怎样流动，文字怎样显示，输入法怎样把文字交给应用     |
| [（八）网络如何到达应用](08-network.md)                | 设备、地址、路由、DNS 和 VPN/TUN 怎样影响应用通信        |
| [（九）挂起、恢复与关机](09-power.md)                  | 系统怎样暂停、回来或退出，哪些资源需要重新准备或释放     |
...
以后读日志、设备节点或服务状态时，也可以这样问自己：这份输出来自哪个组件？它能证明哪一步发生了？还缺哪一段证据？带着这些问题读[启动篇](02-boot.md)，从固件交出控制权的地方开始。
